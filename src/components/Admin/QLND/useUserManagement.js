...
export default function useUserManagement() {
  ...
  const fetchUsers = async (pageNo = 1, pageSize = 10, keyword = null) => {
    console.log(
      `[useUserManagement] Fetching users with pageNo: ${pageNo}, pageSize: ${pageSize}, keyword: ${keyword}`
    );
    try {
      ...
      console.error(
        "[useUserManagement] Lỗi khi lấy danh sách người dùng:",
        err
      );
      users.value = [];
    }
    ...
    try {
      ...
      if (detailData.value) {
        if (Array.isArray(detailData.value) && detailData.value.length > 0) {
          ...
          Array.isArray(detailData.value.data) &&
          detailData.value.data.length > 0
        ) {
        ...
        console.log(
          "[useUserManagement] User detail fetched successfully:",
          userDetail.value
        );
      } else {
        userDetail.value = null;
        console.log(
          "[useUserManagement] No user detail data returned for ID:",
          userId
        );
      }
    } catch (err) {
      console.error(
        "[useUserManagement] Lỗi khi lấy thông tin chi tiết người dùng:",
        err
      );
      userDetail.value = null;
    }
    ...
    try {
      await performActionAPI("WBH_US_CRT_CREATE_ACCOUNT", {
        params: {
          ...
          p_trangthai:
            userData.trangthai !== undefined ? userData.trangthai : 1,
        },
      });

      console.log(
        "[useUserManagement] Create user API response:",
        actionData.value
      );
  ...
  const updateUser = async (userId, userData) => {
    console.log(
      `[useUserManagement] Updating user ID: ${userId} with data:`,
      userData
    );
    ...
    try {
      ...
      console.log(
        "[useUserManagement] Update user API response:",
        actionData.value
      );
  ...
  const updateUserRole = async (userId, role) => {
    console.log(
      `[useUserManagement] Role update disabled for user ID: ${userId}`
    );
    actionResult.value = {
      success: false,
  ...
  // Cập nhật toàn bộ thông tin người dùng bao gồm vai trò và trạng thái
  const updateUserComplete = async (userId, userData) => {
    console.log(
      `[useUserManagement] Updating complete user info for ID: ${userId} with data:`,
      userData
    );
    ...
    try {
      ...
      console.log(
        "[useUserManagement] Update basic info API response:",
        actionData.value
      );
      ...
      if (actionData.value) {
        ...
        if (result && result.rtn_value !== undefined) {
          basicUpdateSuccess = result.rtn_value === 0;
          ...
          basicUpdateSuccess = result.affected_rows > 0;
        }
      }

      // Bước 2: Cập nhật trạng thái nếu có thay đổi
      let statusUpdateSuccess = true;
      if (userData.trangthai !== undefined) {
        const statusSuccess = await updateUserStatus(
          userId,
          userData.trangthai
        );
        if (!statusSuccess) {
      ...
      if (basicUpdateSuccess && statusUpdateSuccess) {
        actionResult.value = {
          success: true,
          message:
            "Cập nhật thông tin người dùng thành công (vai trò không thay đổi)",
        };
        return true;
      } else {
        actionResult.value = {
          success: false,
          message: "Một số thông tin không được cập nhật thành công",
        };
        return false;
      }
    } catch (err) {
      console.error(
        "[useUserManagement] Lỗi khi cập nhật toàn bộ thông tin người dùng:",
        err
      );
      actionResult.value = {
        success: false,
