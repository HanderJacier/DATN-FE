...
export default function useCartManagement() {
  ...
  const loadCart = () => {
    try {
      ...
      console.log("[useCartManagement] Cart loaded from localStorage:", cart.value)
    } catch (error) {
      console.error("[useCartManagement] Error loading cart from localStorage:", error)
      cart.value = []
    }
  }
  ...
  const saveCart = () => {
    try {
      ...
      console.error("[useCartManagement] Error saving cart to localStorage:", error)
      return false
    }
  }
  ...
  const addToCart = (product, quantity = 1) => {
    try {
      ...
      const existingIndex = cart.value.findIndex(
        (item) => item.id === (product.id_sp || product.id) && item.variant === formatVariant(product),
      )
      ...
      if (existingIndex !== -1) {
        ...
        const cartItem = {
          ...
          originalPrice: product.dongia || product.originalPrice || product.price,
          image: product.anhgoc || product.image || "/placeholder.svg?height=100&width=100",
          variant: formatVariant(product),
          quantity: quantity,
          selected: false,
          category: product.loaiTen || product.category || "",
          brand: product.thuonghieuTen || product.brand || "",
          stockQuantity: product.soluong_kho || product.stockQuantity || 999,
        }
  ...
  const updateQuantity = (productId, variant, quantity) => {
    try {
      const itemIndex = cart.value.findIndex((item) => item.id === productId && item.variant === variant)
  ...
  const removeFromCart = (productId, variant) => {
    try {
      const filteredCart = cart.value.filter((item) => !(item.id === productId && item.variant === variant))
  ...
  const updateSelected = (productId, variant, selected) => {
    try {
      const item = cart.value.find((item) => item.id === productId && item.variant === variant)
      if (item) {
  ...
  const removeSelectedItems = () => {
    try {
      ...
      console.error("[useCartManagement] Error removing selected items:", error)
      return false
    }
  }
  ...
  const isInCart = (productId, variant = "Mặc định") => {
    return cart.value.some((item) => item.id === productId && item.variant === variant)
  }
