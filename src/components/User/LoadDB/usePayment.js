...
export default function usePayment() {
  ...
  const processPayment = async (paymentData) => {
    ...
    try {
      ...
      // Validate input data
      if (!paymentData || !paymentData.items || !Array.isArray(paymentData.items) || paymentData.items.length === 0) {
        throw new Error("Dữ liệu đơn hàng không hợp lệ")
      }

      if (!paymentData.customerInfo || !paymentData.customerInfo.name || !paymentData.customerInfo.phone) {
