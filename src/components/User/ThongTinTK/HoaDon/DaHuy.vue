<template>
  <div class="container my-5">
    <div class="row">
      ...
      <!-- Nội dung -->
      <div class="col-md-9">
        <h4 class="fw-bold mb-4">Đơn hàng đã hủy</h4>
        <Historybar />
          ...
          <div class="row g-3 align-items-center">
            ...
            <div class="col-md-6 d-flex justify-content-end align-items-center gap-2">
              <span class="text-muted small me-2">
                {{ filteredOrders.length }} đơn (Đã hủy) — trang {{ pageNo }} / {{ totalPages }}
              </span>
        ...
        <div v-if="!idTk" class="alert alert-warning">
          Bạn chưa đăng nhập hoặc thiếu <code>id_tk</code>. Vui lòng đăng nhập để xem đơn hàng đã hủy.
        </div>
        ...
        <!-- Bảng hóa đơn (chỉ “Đã hủy”) / Empty-state -->
        <div v-else>
          <div v-if="filteredOrders.length === 0" class="border rounded-4 p-4 mb-4 bg-white text-center">
            <i class="bi bi-inbox fs-1 text-muted d-block mb-2"></i>
            <h6 class="mb-1">Chưa có đơn hàng đã hủy</h6>
            <p class="text-muted mb-3">Khi đơn của bạn bị hủy, chúng sẽ hiển thị tại đây.</p>
                ...
                <tr v-for="order in paginatedOrders" :key="order.id_hd">
                  ...
                  <td>
                    <div class="btn-group btn-group-sm">
                      ...
                      <!-- Không hiển thị nút Hủy/Mua lại ở trang Đã hủy -->
                    </div>
                  </td>
                </tr>
              </tbody>
...
import useHoaDonTheoTaiKhoan from '../../LoadDB/HoaDon' // chỉnh đường dẫn theo dự án
...
export default {
  ...
  setup() {
    ...
    // ✅ Chỉ lấy các order có trạng thái "Đã hủy", rồi mới filter theo keyword
    const filteredOrders = computed(() => {
      let filtered = orders.value.filter(o => o?.trangthai === 'Đã hủy')
    ...
    // Nếu id_tk thay đổi ở tab khác → tự reload danh sách
    const onStorage = (e) => {
